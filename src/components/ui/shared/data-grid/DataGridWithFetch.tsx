...
import {
   ColumnDef, flexRender, getCoreRowModel, getFilteredRowModel, getSortedRowModel, PaginationState, SortingState, useReactTable,
...
export interface DataGridProps<T extends Array<any>> {
   ...
   fetchOptionHandler: { set: (pageIndex: number, limit: number, searchValue: string) => void, get: () => { offset: number, limit: number, search: string } }
}

export const useDataGridFetchOptions = () => {
   const [fetchOptions, setFetchOptions] = useState<{ offset: number, limit: number, search: string }>({ offset: 0, limit: 0, search: '' })
   return {
      get: () => fetchOptions,
      set: (pageIndex: number, limit: number, searchValue: string) => setFetchOptions({
         offset: pageIndex * limit, limit: limit, search: searchValue,
      }),
   }
}
...
export function DataGrid<T extends Array<any>>(props: DataGridProps<T>) {
   ...
   const pagination = useMemo(
      () => {
         fetchOptionHandler.set(pageIndex, pageSize, globalFilter)
         return {
            pageIndex,
            pageSize,
         }
   ...
   return (
      <div>
         ...
         {/* Table */}
         <div className="flex flex-col mt-4 overflow-y-hidden overflow-x-auto border rounded-md">
            <div className="relative">
               <div className="align-middle inline-block min-w-full relative">
                  <LoadingScreen show={isFetching} className="absolute" />
                  <table
                     className="min-w-full divide-y divide-gray-200 overflow-x-auto relative"
                  >
                        ...
                        <tr key={headerGroup.id}>
                           {headerGroup.headers.map((header, index) => (
                              <th
                                 ...
                                 scope="col"
                                 className={cn(
                                    "px-6 h-12 text-left text-sm font-bold",
                                    {
            ...
            <div className="flex w-full items-center gap-2 justify-between border-t p-2 mt-2">
               ...
               <div className="flex flex-none items-center gap-2">
                  ...
                  <Select
                     ...
                     options={[Number(_limit), 10, 20, 30, 40, 50].map(pageSize => ({ value: pageSize, label: `Show ${pageSize}` }))}
